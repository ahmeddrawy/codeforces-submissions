...
//    cout << ceil(-5*1.0/2)<<endl;
    vector<int > vec(n);
    vector<int > postiveodds;
    vector<int > negativeodds;
    MOD = 2;
    ll s = 0 ;
    for (int i = 0; i <n ; ++i) {
        int x ; cin >> x;
        ...
        if(mod(x) !=0){
            if(x<0)
                negativeodds.push_back(i);
            else
                postiveodds.push_back(i);
        }
    ...
    if(s!=0){
        if(s>0){
            int x = abs(s);
            for (int i = 0; i <sz(negativeodds) && x; ++i,x--) {
                vec[negativeodds[i]]-=1;
            }
        }
        else {
            int x = abs(s);
            for (int i = 0; i <sz(postiveodds) && x; ++i,x--) {
                vec[postiveodds[i]]+=1;
            }
        }
    }
